<template>
  <div class="px-6 md:px-12 py-16 bg-[#0A0A0B] min-h-[calc(100vh-56px)] text-white font-sans relative overflow-hidden">
    ...
    <div class="absolute bottom-0 left-0 w-[600px] h-[600px] bg-gradient-to-tr from-[#3B82F6]/10 to-transparent rounded-full blur-[120px] pointer-events-none -z-10"></div>

    <div class="max-w-5xl mx-auto relative z-10">
      <!-- Header Section -->
      <div class="mb-20">
        <h1 class="text-4xl font-semibold tracking-[-0.04em] text-white">Intelligence Base</h1>
      ...
      <div class="mb-24 bg-[#050505]/80 border border-[#222] rounded-3xl p-10 overflow-hidden relative shadow-2xl backdrop-blur-xl group">
        <div class="absolute top-0 right-0 w-64 h-64 bg-white/[0.01] blur-3xl rounded-full -translate-y-1/2 translate-x-1/2 group-hover:bg-[#A855F7]/10 transition-colors duration-1000 -z-10"></div>
        ...
        <form @submit.prevent="createItem" class="space-y-10 relative z-10">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div class="space-y-4">
              ...
              <div class="relative group/select">
                ...
                <div class="absolute right-5 top-1/2 -translate-y-1/2 pointer-events-none text-[#888] group-hover/select:text-white transition-colors">
          ...
          <div v-else class="space-y-4">
            <div class="relative group/upload h-40 bg-[#111] border border-dashed border-[#333] hover:border-[#A855F7]/50 rounded-2xl transition-all flex flex-col items-center justify-center cursor-pointer overflow-hidden shadow-inner">
              ...
              <div class="text-center group-hover/upload:scale-105 transition-transform duration-500">
      ...
      <div class="space-y-10 relative z-10">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 relative">
          <div v-for="item in items" :key="item.id" class="bg-[#050505]/90 border border-[#222] hover:border-[#444] rounded-3xl p-8 group hover:-translate-y-1 transition-all shadow-lg overflow-hidden relative backdrop-blur-xl">
            <!-- Subtle hover glow for cards -->
            <div class="absolute top-0 right-0 w-32 h-32 bg-white/[0.01] blur-2xl group-hover:bg-[#A855F7]/5 transition-all duration-500 pointer-events-none"></div>

             <div class="flex items-start justify-between mb-8 relative z-10">
               <div class="w-14 h-14 rounded-2xl bg-[#111] border border-[#222] flex items-center justify-center group-hover:bg-white group-hover:border-white transition-all duration-500 shadow-inner">
                 <svg v-if="item.item_type === 'file'" class="w-6 h-6 text-[#888] group-hover:text-black transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M7 21h10a2 2 0 002-2V9.414a1 1 0 00-.293-.707l-5.414-5.414A1 1 0 0012.586 3H7a2 2 0 00-2 2v14a2 2 0 002 2z"></path></svg>
                 <svg v-else class="w-6 h-6 text-[#888] group-hover:text-black transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z"></path></svg>
               </div>
               <button @click="deleteItem(item.id)" class="text-[#888] bg-[#111] hover:bg-red-500/10 hover:text-red-400 p-2.5 rounded-xl border border-[#222] hover:border-red-500/20 transition-all shadow-sm">
